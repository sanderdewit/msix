...
| **Bug fix** | `Get-MsixHeuristicFindings` — `$hasLegacyCtxMenu` (ShellExt idempotency cross-check) was checking `$exts` (Package-level `Package/Extensions`) for `windows.fileExplorerClassicContextMenuHandler`; after the level fix, desktop9 extensions live in `$appExts` (Application-level); changed to check `@($exts) + @($appExts)` so a second run of `Get-MsixCompatibilityReport` suppresses the ShellExt finding and `Invoke-MsixAutoFixFromAnalysis` no longer re-applies `Add-MsixLegacyContextMenu` (which would add duplicate COM class declarations, causing MakeAppx to fail) |
| **Bug fix** | `Get-MsixHeuristicFindings` — `$hasLoaderOv` (LoaderSearchPathOverride idempotency cross-check) was scanning only `$exts` (Package-level); `uap6:LoaderSearchPathOverride` lives at Application-level after the level fix; changed to scan `@($exts) + @($appExts)` for the child override element; second-run idempotency now works correctly |
| **Bug fix** | `Get-MsixStaticAnalysis` — PSF detection used `($apps.Executable -match 'PsfLauncher').Count -gt 0`; when `$apps` contains a single element, PowerShell's `-match` on a scalar returns `$true`/`$false` (not a filtered array), so `.Count` is always `1`, making every single-application package appear to use PsfLauncher; fixed to use `@($apps \| Where-Object { $_.GetAttribute('Executable') -match 'PsfLauncher' }).Count -gt 0`; additionally, a config.json presence check now guards the finding — bare PsfLauncher without config.json is not a valid PSF install |
| **Bug fix** | `Add-MsixLegacyContextMenu` — idempotency guard added: if the COM class CLSID is already declared anywhere in the manifest, the mutate block returns early instead of appending a duplicate `com:Extension`/`desktop9:Extension` pair (would cause MakeAppx schema violation on re-run) |
